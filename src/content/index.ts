...
if (window.__scrollSyncContentLoaded) {
  // уже сидим на странице - второй раз listeners не вешаем
} else {
...
function bootContentScript(): void {
  ...
  function emitScrollUpdate(): void {
    ...
    void chrome.runtime
      ...
      .catch(() => {
        // расширение перезагрузили или sw ещё спит - не страшно
      });
  }
  ...
  function discoverScrollContainers(): void {
    // todo: сейчас только знакомые селекторы - на google/spa часто мимо
    const selectors = [
      '.kix-appview-editor',
      '#docs-editor',
